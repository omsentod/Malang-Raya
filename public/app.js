...
// Package Data will be fetched from Python API
let packagesData = [];
...
function setupFormHandlers() {
    ...
    // Form submit integrated with Laravel + Python API
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        
        const submitBtn = form.querySelector('button[type="submit"]');
        submitBtn.textContent = 'Memproses FCM...';
        submitBtn.disabled = true;

        try {
            const formData = new FormData();
            formData.append('workflow', 'budget');
            formData.append('persons', appState.budgetData.participants);
            formData.append('duration', appState.budgetData.duration);
            formData.append('budget', appState.budgetData.totalBudget);
            ...
            if (response.ok) {
                // Transform JSON dari Python ke struktur PackagesData
                packagesData = data.map((pkg, i) => {
                    const mappedType = {
                        'HEMAT': { type: 'hemat', badgeColor: 'bg-green-500' },
                        'BALANCED': { type: 'balanced', badgeColor: 'bg-blue-500' },
                        'PREMIUM': { type: 'premium', badgeColor: 'bg-amber-500' }
                    };
                    const styling = mappedType[pkg.kategori] || mappedType['HEMAT'];
                    
                    return {
                        type: styling.type,
                        badge: pkg.kategori.toUpperCase(),
                        badgeColor: styling.badgeColor,
                        status: 'Rekomendasi Terbaik ' + (i+1),
                        title: pkg.kategori + ' Options',
                        hotel: {
                            name: pkg.hotel_nama,
                            fullName: pkg.hotel_nama,
                            price: pkg.hotel_harga,
                            coords: '-7.9826, 112.6308', lat: -7.9826, lng: 112.6308
                        },
                        wisata: {
                            name: pkg.wisata_nama,
                            fullName: pkg.wisata_nama,
                            price: pkg.wisata_harga,
                            coords: '-7.9826, 112.6308', lat: -7.9826, lng: 112.6308
                        },
                        makan: {
                            name: pkg.kuliner_nama,
                            fullName: pkg.kuliner_nama,
                            price: pkg.kuliner_harga,
                            coords: '-7.9826, 112.6308', lat: -7.9826, lng: 112.6308
                        },
                        totalHotel: pkg.cost_akomodasi,
                        totalWisata: pkg.cost_wisata,
                        totalMakan: pkg.cost_kuliner,
                        transportConfig: pkg.cost_transport,
                        totalPrice: pkg.total_cost
                    }
                });
                ...
                alert('Terdapat kesalahan: ' + (data.error || 'Server error'));
            }
        } catch (error) {
            console.error('Request Gagal', error);
            alert('Gagal menyambung ke server');
        } finally {
            submitBtn.textContent = 'Cari Paket Wisata';
